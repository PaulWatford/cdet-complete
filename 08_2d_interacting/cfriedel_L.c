/* cfriedel_L.c (v122) -- generalized to arbitrary L -- the elementary frozen Friedel object rho(0,r), ported to C.
 *
...
static double eps(int L, int kx, int ky, int kz) {
...
double cfriedel_rho_L(int L, double mu, int x, int y, int z) {
    double s = 0.0; long N = (long)L*L*L;
    for (int kx = 0; kx < L; kx++)
        for (int ky = 0; ky < L; ky++)
            for (int kz = 0; kz < L; kz++)
                if (eps(L, kx, ky, kz) <= mu + 1e-9)
                    s += cos(2*PI*(kx*x + ky*y + kz*z)/(double)L);
    return s/(double)N;
}
double cfriedel_rho(int x, int y, int z) { return cfriedel_rho_L(6, 1.0, x, y, z); } /* L=6 mu=1 default */

int cfriedel_occupied_L(int L, double mu) {
    int n = 0;
    for (int kx = 0; kx < L; kx++)
        for (int ky = 0; ky < L; ky++)
            for (int kz = 0; kz < L; kz++)
                if (eps(L, kx, ky, kz) <= mu + 1e-9) n++;
    return n;
}
int cfriedel_occupied(void) { return cfriedel_occupied_L(6, 1.0); }
int cfriedel_integer_spectrum(int L) { /* 1 if cos(2pi/L) rational (crystallographic L in {1,2,3,4,6}) */
    return (L==1||L==2||L==3||L==4||L==6) ? 1 : 0;
}
...
static int selftest(void) {
    ...
    for (int i = 0; i < nref; i++) {
        double got = cfriedel_rho_L(6, 1.0, ref[i].x, ref[i].y, ref[i].z);
    ...
    if (cfriedel_occupied_L(6,1.0) != 156) { fail++; printf("OCC FAIL: %d != 156\n", cfriedel_occupied_L(6,1.0)); }
    if (fail == 0)
...
int main(int argc, char **argv) {
    ...
    if (argc >= 2 && !strcmp(argv[1], "occ")) { printf("occupied modes (eps<=1): %d\n", cfriedel_occupied_L(6,1.0)); return 0; }
    if (argc >= 4 && !strcmp(argv[1], "occL")) { int Lp=atoi(argv[2]); double mu=atof(argv[3]);
        printf("L=%d N=%d occupied(eps<=%.3f)=%d integer-spectrum=%d\n", Lp, Lp*Lp*Lp, mu, cfriedel_occupied_L(Lp,mu), cfriedel_integer_spectrum(Lp)); return 0; }
    if (argc >= 2 && !strcmp(argv[1], "scan")) {
        printf("# L  N     integer-spectrum(crystallographic)  occupied(eps<=1)\n");
        for (int Lp=2; Lp<=14; Lp++) printf("  %-3d %-6d %-3d  %d\n", Lp, Lp*Lp*Lp, cfriedel_integer_spectrum(Lp), cfriedel_occupied_L(Lp,1.0));
        return 0; }
    if (argc >= 5 && !strcmp(argv[1], "r")) {
        int x = atoi(argv[2]), y = atoi(argv[3]), z = atoi(argv[4]);
        int Lp = (argc >= 6) ? atoi(argv[5]) : 6;
        double mu = (argc >= 7) ? atof(argv[6]) : 1.0;
        printf("rho_L%d(0,(%d,%d,%d)) = %+.10f\n", Lp, x, y, z, cfriedel_rho_L(Lp, mu, x, y, z));
        return 0;
    }
    if (argc >= 3 && !strcmp(argv[1], "map")) {
        int z = atoi(argv[2]);
        int Lp = (argc >= 4) ? atoi(argv[3]) : 6;   /* optional lattice size, default 6 */
        double mu = (argc >= 5) ? atof(argv[4]) : 1.0;
        printf("# elementary frozen Friedel sign-map rho(0,(x,y,%d)) for L=%d mu=%.2f; +/- 0=node o=origin\n", z, Lp, mu);
        printf("     ");
        for (int x = 0; x < Lp; x++) printf("x%d ", x);
        printf("\n");
        for (int y = 0; y < Lp; y++) {
            printf("y%d:  ", y);
            for (int x = 0; x < Lp; x++) {
                if (x == 0 && y == 0 && z == 0) { printf(" o "); continue; }
                double v = cfriedel_rho_L(Lp, mu, x, y, z);
    ...
    fprintf(stderr, "usage: cfriedel_L test | r x y z | map z [L] [mu] | occ | occL L mu | scan\n");
    return 1;
}

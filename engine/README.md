> **Role in the archive (v147 consolidation):** this is the FROZEN REFERENCE engine — the 194/194 validation
> anchor that every result in the archive is checked against. Its validated numerics are never altered; that is
> what makes the archive trustworthy. Efficiency targets `make fast` (-O3 -march=native) and `make omp` (OpenMP)
> are available. The capability-carrying production engine is `08_2d_interacting/cdet_planewave_engine.c` (the
> hybrid), which validates == this reference bit-for-bit at L=6.

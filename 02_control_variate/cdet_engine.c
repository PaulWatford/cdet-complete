...
double D_corr(const Vertex *V, int n,
 ...
 g0_fn g0, void *ctx) {
 int rs[MAXDIM], cs[MAXDIM]; double rt[MAXDIM], ct[MAXDIM];
...
double C_V(const Vertex *V, int n,
 ...
 int N = 1 << n;
 double *Dc = malloc(sizeof(double) * N);
 ...
 Dc[mask] = D_corr(sub, m, site_out, tau_out, site_in, tau_in, g0, ctx);
